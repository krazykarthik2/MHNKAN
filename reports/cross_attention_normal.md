...
## 5. Complexity & VRAM Analysis

* **Sequence Time Complexity**: $O(L \cdot M)$ where $L$ is sequence length and $M$ is memory count. It scales strictly linearly with the sequence length $L$ since tokens process independently through the memory bank, avoiding the $O(L^2)$ sequence self-attention bottleneck.
* **Storage Space Complexity (Parameters)**: $O(d \cdot M)$ parameters to store the Key and Value matrices, which is constant and independent of sequence length.
* **Run-time VRAM Footprint Scaling**:
  - **Modern Hopfield Network (MHN)**: Stores pattern states dynamically in the activation/KV cache. As sequence length $L$ grows, active VRAM usage scales dynamically as $O(d \cdot L)$ and is cleared when the sequence ends.
  - **Static Cross-Attention (`cross_attn_normal`)**: Stores patterns as persistent model weights. The VRAM footprint is permanently fixed at $O(d \cdot M)$ and never scales with sequence length $L$.
